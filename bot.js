...
client.on("guildMemberAdd", async member => {
  ...
  if (!skanal31) return;
  skanal31.send(`<a:gu:753638233434488842> \`${ member.user.tag }\` Adlı Kullanıcı Sunucuya Katıldı. \`${sayac}\` Kullanıcı Olmaya \`${sayac - member.guild.members.size}\` Kullanıcı Kaldı ! <a:hg:753218809246253096>  ` );
});
...
client.on("guildMemberRemove", async member => {
  ...
  const skanal31 = member.guild.channels.find("name", skanal9);
  if (!skanal31) return;
  skanal31.send(`<a:gu:753638233434488842> \`${  member.user.tag }\`Adlı Kullanıcı Sunucudan Ayrıldı. \`${sayac}\` Kullanıcı Olmaya \`${sayac - member.guild.members.size}\` Kullanıcı Kaldı ! <a:bb:753218730867032114> `);
});
...
async function handleVideo(video, message, voiceChannel, playlist = false) {
  ...
  .setColor("RANDOM")
  .setTitle("** Müzik Sistemi**", `https://i.postimg.cc/BnQCzh2s/861acd45b3d34f97265ac7161fbb6763.png`)
